 ...
 * @author firem
 */
@WebServlet(name = "BaseUserController", urlPatterns = {"/BaseUserController"})
public abstract class BaseUserController extends HttpServlet {
    ...
    private void listNotification(HttpServletRequest request){
        ...
        if(customer.isStatus()){
            ...
            for (Payment payment : listPaymentToPay) {
                if(payment.getContract().getId() == contract.getId()){
                    listPayment.add(payment);
                }
            }
        }
        session.setAttribute("listPaymentToPay", listPayment);
